...
class AutoEncoder(nn.Module):
    ...
    def get_hmc_lat2(self, z1, normalized_attacked, combiner_cells_enc, combiner_cells_s, dist, ftr):
        ...
        for i in range(n_steps):
            ...
            #print("div", div)
            z = z.view(z_shape).requires_grad_(True)#.detach().requires_grad_(True)  # Prepare for next iteration
            del mask, logp_new, logp_current, p_new, grad, log_post, log_p_x, x_flat, x_mean_flat, 
    ...
    def forward(self, x):
        ...
        for q, p, log_q_conv, log_p_conv in zip(all_q, all_p, all_log_q, all_log_p):
            #print("self.with_nf", self.with_nf)
        ...
        return logits, log_q, log_p, kl_all, kl_diag, all_lat_rep
