...
if(attck_type == "grill_cos_kf_SS"):
    ...
    for step in range(10000):
        ...
        for layer, adv_output in adv_layerwise_outputs.items():
            #print("adv_output.shape", adv_output.shape)
            orig_output = orig_layerwise_outputs[layer]
            loss += (1 - cos(adv_output, orig_output)**2 * cond_nums_normalized[counter])**2
            counter+=1
        layerwise_outputs.clear()      
        lat_loss = 0
        for i in range(len(adv_latent_reps)):
            lat_loss += (1-cos(adv_latent_reps[i], orig_latent_reps[i]))**2
...
if(attck_type == "grill_l2_kf_po"):
    ...
    for step in range(100):
        for source_im in big_tensor:
            ...
            total_loss = -1 * loss * criterion(adv_output, orig_output)  #* lat_loss
...
if(attck_type == "la_wass_kf_cr_SS"):
    adv_div_list = []
    adv_mse_list = []
    all_adv_div_list = []
    allAvgEpochLosses = []
    ...
    for step in range(10000):
        ...
        loss = 0
        for i in range(len(adv_latent_reps)):
            loss = loss + wasserstein_distance_cr(adv_latent_reps[i], orig_latent_reps[i])
...
if(attck_type == "la_cos_kf_cr_SS"):
    ...
    for step in range(10000):
        ...
        loss = 0
        for i in range(len(adv_latent_reps)):
            loss = loss + (1 - cos(adv_latent_reps[i], orig_latent_reps[i]))**2
