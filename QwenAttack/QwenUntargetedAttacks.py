...
blackHole, boat, cheetah, light, walker, nature, 
...
snapshot_download(
    ...
    local_dir="/home/luser/vlmAttack/Gemma3-4b",
...
parser.add_argument('--sampleName', type=str, default="nature", help='Segment index')
...
MODEL_PATH = "QwenAttack/Qwen2.5-VL-7B-Instruct"

#IMAGE_PATH = "/home/luser/vlm_learn/dataSamples/dogs68.jpg"

IMAGE_PATH = f"QwenAttack/dataSamples/{sampleName}68.jpg"
...
def adam_attack_on_patches(
    ...
    for step in range(num_steps):
        ...
        if attck_type=="grill_wass":
            loss = getGrillWassLoss(outputs, outputsN)
        if attck_type=="grill_cos":
            loss = getGrillCosLoss(outputs, outputsN)
        if attck_type=="OA_l2":
            loss = getOALoss(outputs, outputsN)
        if attck_type=="OA_wass":
            loss = getOAWassLoss(outputs, outputsN)
        ...
        if max(lossesList) < loss.item():
            ...
            np.save(f"QwenAttack/outputsStorage/convergence/{sampleName}/vlm_attack_type__given_attackType_{attck_type}_lr_{learningRate}_epsilon_{desired_norm_l_inf}_.npy", lossesList)
...
def main():
    ...
    save_adv_image_from_pixel_values(
        ...
        inputs,
        processor,
        save_path=f"QwenAttack/outputsStorage/{sampleName}/adv_imagegiven_attackType_{attck_type}_lr_{learningRate}_epsilon_{desired_norm_l_inf}_.png",
    )
    ...
    # SAVE TO TEXT FILE
    # --------------------------------------------------------
    filename = f"QwenAttack/outputsStorage/{sampleName}/output_given_attackType_{attck_type}_lr_{learningRate}_epsilon_{desired_norm_l_inf}_.txt"
